import time
import color as col

from ..gsgr.conditions import OR, cm, impact, pickup, sec
from ..gsgr.enums import Attachment
from ..gsgr.config import PID, cfg, configure
from ..gsgr.movement import (
    gyro_drive,
    gyro_set_origin,
    gyro_turn,
    gyro_wall_align,
    hold_attachment,
    run_attachment,
    stop_attachment,
)
import hub, motor

display_as = 6
color = col.GREEN
config = configure().gyro_drive(PID(1.2, 0, -0.2))


def run():
    # Set Gyro Origin
    run_attachment(Attachment.FRONT_LEFT, 100, 2.25, stall=True, await_completion=False)
    gyro_wall_align(0.2)
    gyro_drive(0, 700, cm(68), accelerate=10, decelerate=30)
    gyro_turn(-90)
    gyro_drive(-90, 700, cm(34.5), accelerate=10, decelerate=30)
    gyro_turn(0)
    gyro_drive(0, -500, cm(8), accelerate=10, decelerate=30)
    run_attachment(Attachment.FRONT_LEFT, -200, 1.5, stall=True)
    gyro_drive(0, 500, cm(2.8), accelerate=10, decelerate=30)
    run_attachment(Attachment.FRONT_LEFT, 300, 0.9, stall=True)
    gyro_drive(0, 500, cm(4), accelerate=10, decelerate=30)
    run_attachment(Attachment.FRONT_LEFT, 300, 1, stall=True)
    gyro_drive(0, 500, cm(5), accelerate=10, decelerate=30)
    gyro_drive(0, 1000, sec(2), pid=PID(0, 0, 0))
    time.sleep(2)
    run_attachment(Attachment.FRONT_RIGHT, 1000, 5)
    run_attachment(Attachment.FRONT_RIGHT, -600, 1.3, stall=True)
    hold_attachment(Attachment.FRONT_LEFT)
    gyro_drive(0, -500, cm(13), accelerate=10, decelerate=30)
    run_attachment(Attachment.BACK_RIGHT, -600, 1.3, stall=True)
    run_attachment(Attachment.BACK_RIGHT, 600, 1.3, stall=True)
