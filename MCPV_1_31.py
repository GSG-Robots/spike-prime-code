...
class Run:
    def __init__(self, brick:PrimeHub, engines:list[str]=['D','C','B','E'], lightSensors:list[str]=['A','F'],
    correctionValues:list[float]=[0.5,0,0,0,0,0,1,1,1], tireRadius:float=2.6, lightBlackValue:int=10,
    lightMiddleValue:int=50, turningDegreeTolerance:int=2):
        '''
        Initiation of Run
        ...
        if(self.gearSelector.get_position()<=90 or self.gearSelector.get_position()>=270):
    ...
    def selectGear(self, targetGear:int):
        ...
        if(self.selectedGear<targetGear):
            self.gearSelector.run_to_position(int(58*(targetGear-1)),'clockwise',100)
        elif(self.selectedGear>targetGear):
            self.gearSelector.run_to_position(int(58*(targetGear-1)),'counterclockwise',100)
        self.selectedGear=targetGear


    def driveAttachment(self, attachmentIndex:int, speed:int, resistance:bool=False, duration:float=0, degree:float=0):
        '''
        ...
        if (duration!=0):
            ...
            self.driveShaft.run_for_degrees(int(degree*(speed/fabs(speed))),int(fabs(speed)))
        else:
    ...
    def CheckEndingCondition(self, endingCondition:int, endingValue:int):
        ...
        if (endingCondition==0):
            ...
            print(((abs(self.rightMotor.get_degrees_counted())+abs(self.leftMotor.get_degrees_counted()))/360)\
            *pi*self.tireRadius)
            if (((abs(self.rightMotor.get_degrees_counted())+abs(self.leftMotor.get_degrees_counted()))/360)\
            *pi*self.tireRadius>=endingValue):
            ...
            if (self.frontLightSensor.get_reflected_light()<self.lightBlackValue+5 or\
            self.backLightSensor.get_reflected_light()<self.lightBlackValue+5):
                return False
        elif (endingCondition==3):
            if (endingValue-self.turningDegreeTolerance<=self.brick.motion_sensor.get_yaw_angle()<=\
    ...
    def CalculateDeceleration(self, speed:int,endSpeed:float,distance:float):
        ...
        '''
        if ((((self.rightMotor.get_degrees_counted()+self.rightMotor.get_degrees_counted())/720))\
        >=fabs((self.decelerationCounter*distance/50))):
            self.decelerationCounter+=1
        return int((speed*(50-self.decelerationCounter))/50)


    def GyroDrive(self, speed:int, degree:int, endingCondition:int, endingValue:int=0, p_correction:int=0,
    i_correction:int=0, d_correction:int=0, acceleration:int=0, deceleration:int=0, 
    attachmentStart:list[int]=[0,0,0], attachmentStop:int=0):
        '''
        PID Gyro-Drive
        ...
        if(attachmentStart[1]!=0 or attachmentStop!=0):
            while(self.CheckEndingCondition(endingCondition,endingValue)):
                ...
                integral+=errorValue
                corrector=integral*i_correction+differential*d_correction+errorValue*p_correction
                lastError=errorValue
                self.drivingMotors.start_tank(int(self.CalculateAcceleration(speed+corrector,acceleration)),
                int(self.CalculateAcceleration(speed-corrector,acceleration)))
                if(attachmentStart[1]!=0 and not attachmentStarted and self.timer.now()>=attachmentStart[1]):
                    self.driveAttachment(attachmentStart[0],attachmentStart[2])
                    attachmentStarted=True
                if(attachmentStop!=0 and not attachmentStopped and self.timer.now()>=attachmentStop):
                    self.stopAttachment()
        ...
        if (deceleration!=0):
            while(self.decelerationCounter<=50):
                ...
                corrector=integral*i_correction+differential*d_correction+errorValue*p_correction
                lastError=errorValue
                self.drivingMotors.start_tank(self.CalculateDeceleration(speed+corrector),
                self.CalculateDeceleration(speed-corrector))
        self.drivingMotors.stop()



    def GyroTurn(self, degree:int, p_correction:int=0, i_correction:int=0, d_correction:int=0,
    attachmentStart:list[int]=[0,0,0], attachmentStop:int=0):
        '''
        PID-Gyro-Tank-Turn
        ...
        degree=degree-360*floor((degree+180)/360)
        if(attachmentStart[1]!=0 or attachmentStop!=0):
            while(not degree-self.turningDegreeTolerance<self.brick.motion_sensor.get_yaw_angle()<degree+self.turningDegreeTolerance):
                errorValue=degree-self.brick.motion_sensor.get_yaw_angle()
                if (abs(errorValue)>180):
                    errorValue-=360
                differential=errorValue-lastError
                ...
                corrector=integral*i_correction+differential*d_correction+errorValue*p_correction
                lastError=errorValue
                self.drivingMotors.start_tank(attachmentStart[2]-corrector,attachmentStart[2]+corrector)
                if(attachmentStart[1]!=0 and not attachmentStarted and self.timer.now()>=attachmentStart[1]):
                    self.driveAttachment(attachmentStart[0],attachmentStart[2])
                    attachmentStarted=True
                if(attachmentStop!=0 and not attachmentStopped and self.timer.now()>=attachmentStop):
    ...
    def LineFollower(self, speed:int, frontSensor:bool, endingCondition:int, endingValue:int=0,leftOfLine:bool=True,
    p_correction:int=0, i_correction:int=0, d_correction:int=0, attachmentStart:list[int]=[0,0,0], attachmentStop:int=0):
        '''
        PID-Linefollower
        ...
        if(attachmentStart[1]!=0 or attachmentStop!=0):
            while(self.CheckEndingCondition(endingCondition,endingValue)):
                errorValue=leftFactor*(lightSensor.get_reflected_light()-self.lightMiddleValue)
                differential=errorValue-lastError
                integral+=lastError
                corrector=integral*i_correction+differential*d_correction+errorValue*p_correction
                lastError=errorValue
...
def MasterControlProgram(brick:PrimeHub, numberOfRuns:int, immedeatePlay:int=0, engines:list[str]=['D','C','B','E'],
    lightSensors:list[str]=['A','F'], correctionValues:list[float]=[0.5,0,0,0,0,0,0,0,0], tireRadius:float=2.3,
    lightBlackValue:int=10, lightMiddleValue:int=50, turningDegreeTolerance:int=2):
    '''
    ...
    if(immedeatePlay==0):
            ...
            if(brick.left_button.is_pressed()):
                while(True):
                    if(brick.right_button.is_pressed()):
                        ...
                        MasterControlProgram(brick,numberOfRuns,selectedRun,engines,lightSensors,
                        correctionValues,tireRadius,lightBlackValue,lightMiddleValue,turningDegreeTolerance)
            ...
            if(brick.right_button.is_pressed()):
                while(True):
                    if(brick.left_button.is_pressed()):
                        ...
                        print("Play run ", selectedRun)
                        MasterControlProgram(brick,numberOfRuns,selectedRun,engines,lightSensors,
                        correctionValues,tireRadius,lightBlackValue,lightMiddleValue,
        ...
        run1=Run(brick, engines,lightSensors,correctionValues,tireRadius,
        lightBlackValue,lightMiddleValue,turningDegreeTolerance)
        ######################################################################################################### Run 1
        print("Starting Run 1")
        run1.GyroDrive(speed=-60,degree=0,endingCondition=0,endingValue=32,p_correction=4)
        run1.GyroDrive(speed=35,degree=2,endingCondition=1,endingValue=1.2,p_correction=4)
        run1.GyroTurn(degree=-48, p_correction=5)
        run1.GyroDrive(speed=-60, degree=-48, endingCondition=0, endingValue=31, p_correction=3)
        ...
        run2=Run(brick,engines,lightSensors,correctionValues,tireRadius,
        lightBlackValue,lightMiddleValue,turningDegreeTolerance)
        ######################################################################################################### Run 2
        print("Starting Run 2")
        run2.GyroDrive(speed=-60, degree=-4, endingCondition=0, endingValue=66, p_correction=3)
        run2.GyroDrive(speed=70, degree= 5, endingCondition=1, endingValue=2, acceleration=2, p_correction=3)
        wait_for_seconds(0.5)
        run2.GyroTurn(degree=15, p_correction=2)
        run2.GyroDrive(speed=-20, degree=15, endingCondition=0, endingValue=2, p_correction=5)
        run2.GyroTurn(degree=30, p_correction=2)
        run2.GyroDrive(speed=-70, degree=30, endingCondition=0, endingValue=15, p_correction=5)
        run2.GyroTurn(degree=-20, p_correction=2)
        run2.GyroDrive(speed=-100, degree=-20, endingCondition=0, endingValue=90, p_correction=4) ## 70
        ...
        run3=Run(brick,engines,lightSensors, correctionValues,tireRadius,
        lightBlackValue,lightMiddleValue,turningDegreeTolerance)
        ######################################################################################################### Run 3
        print("Starting Run 3")

        
        run3.GyroDrive(speed=-60,degree=0,endingCondition=0,endingValue=10,acceleration=1,p_correction=0.4)
        ...
        run3.GyroDrive(speed=-35,degree=-3,endingCondition=0,endingValue=16,acceleration=1, p_correction=0.6)
        ...
        run3.GyroDrive(speed=-10,degree=-0.75,endingCondition=0,endingValue=10,p_correction=0.6)
        ...
        run4 = Run(brick, engines, lightSensors, correctionValues, tireRadius, lightBlackValue, lightMiddleValue, turningDegreeTolerance)
        ######################################################################################################### Run 4#
        ...
        run4.GyroDrive(speed=-80, degree=-90, endingCondition=0, endingValue=31, p_correction=1.2)
        ...
        run5 = Run(brick, engines, lightSensors, correctionValues, tireRadius, lightBlackValue, lightMiddleValue, turningDegreeTolerance)
